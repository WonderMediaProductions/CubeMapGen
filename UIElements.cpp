...
//LPWSTR*                 g_pMainArgVList;                // Argument list for command line processing (same usage as argv)
//LPWSTR*                 g_pActualArgVList;              // Actual list of arguments (might come from a file specified after @)
//
...
int main()
{
	...
	//g_pActualArgVList = g_pMainArgVList = CommandLineToArgvW(GetCommandLineW(), &g_ArgC );
...
//-------------------------------------------------------------------------------------------
void ConsoleOutputW(HANDLE a_ConsoleHandle, WCHAR *a_ConsoleStr, bool writeToStdErr)
{
	DWORD consoleCharWritten;
...
void ConsoleOutputMessageCallback(WCHAR *a_TitleStr, WCHAR *a_MessageStr)
{
	...
	//console output
	ConsoleOutputW(g_ConsoleHandle, messageString, true);
}
	...
	for (iCmdLine = 1; iCmdLine < argCount; iCmdLine++)
	{
		...
		if (WCPrefixCmp(cmdArg, L"--help", &suffixStr) || WCPrefixCmp(cmdArg, L"-help", &suffixStr))
		{
			...
			ConsoleOutput(consoleStdErr,
				...
				" -numFilterThreads:{1-6} Set number of filtering threads\n"
...
void ProcessCommandLineArguements(void)
{
	...
	int argCount;
	LPWSTR* mainArgList = CommandLineToArgvW(GetCommandLineW(), &argCount);
	...
	WCHAR argsValueBuffer[4096];
	WCHAR* argValuePtrs[100];
	...
	if (argCount == 2 && argList[1][0] == '@')
	{
		...
		if (error == 0)
		{
			...
			while (token)
			{
				argList[argCount++] = argFilePtr;
				argFilePtr += MultiByteToWideChar(CP_ACP, 0, token, -1, argFilePtr, 256);
	...
	for (iCmdLine = 1; (iCmdLine < argCount); iCmdLine++)
	{
		...
		{
			g_CubeGenApp.m_CubeMapProcessor.m_NumFilterThreads = min(6, max(1, _wtoi(suffixStr)));
		}
	...
	if (bInvalidOption == true)
	{
		...
		ConsoleOutputW(consoleStdErr, outputStr, m_bHDRShopMode);
	...
	if (bExportCubeDDS || bExportCubeFaces || bExportCross || bExportHDRShopCross)
	{
		...
		while (g_CubeGenApp.m_CubeMapProcessor.GetStatus() == CP_STATUS_PROCESSING)
		{
			...
			{
				//display progress
				ConsoleOutputW(consoleStdErr,
					L"\r                                                                                  \r", m_bHDRShopMode);
				auto msg = g_CubeGenApp.m_CubeMapProcessor.GetFilterProgressString(true);
				ConsoleOutputW(consoleStdErr, msg, m_bHDRShopMode);
			}
		}

		if (m_bHDRShopMode == false)
		{
			ConsoleOutputW(consoleStdErr, L"\nConverting Cubemap to Desired Format.\n", m_bHDRShopMode);  //carriage return
		}
